...
int main(){auto dir=fs::temp_directory_path()/"tscc-program-graph-smoke";fs::remove_all(dir);fs::create_directories(dir);write(dir/"a.ts","import {b} from './b.js'; export const a:number=b+1; function keep(value:number):number{return value;} keep(a);");write(dir/"b.ts","import {a} from './a.js'; export const b:number=1;");
 ...
 write(dir/"shapes.ts","export default function answer():number{return 42;} export const label:string='ok';");write(dir/"imports-good.ts","import answer from './shapes'; import * as shapes from './shapes'; const n:number=answer(); const s:string=shapes.label;");write(dir/"imports-bad.ts","import answer from './shapes'; import * as shapes from './shapes'; const n:string=answer(); const s:number=shapes.label;");ProgramGraph imports_good;if(!imports_good.build({(dir/"imports-good.ts").string()},true))fail("default/namespace import facts were not accepted");ProgramGraph imports_bad;if(imports_bad.build({(dir/"imports-bad.ts").string()},true))fail("default/namespace import mismatch was not rejected");
