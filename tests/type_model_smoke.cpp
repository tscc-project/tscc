...
int main() {
    ...
    const auto mixed=types.named_types.find("Mixed");if(mixed==types.named_types.end()||types.store.kind(mixed->second)!=TypeKind::Object)fail("object alias with unknown members missing");const auto*mixed_req=types.store.property(mixed->second,"required");if(!mixed_req||mixed_req->type!=types.store.string()||mixed_req->optional)fail("Mixed.required:string missing");const auto*mixed_maybe=types.store.property(mixed->second,"maybe");if(!mixed_maybe||mixed_maybe->type!=types.store.unknown()||!mixed_maybe->optional)fail("Mixed.maybe?:unknown missing");const auto*mixed_children=types.store.property(mixed->second,"children");if(!mixed_children||mixed_children->type!=types.store.unknown()||!mixed_children->optional)fail("Mixed.children?:unknown missing");const auto*mixed_count=types.store.property(mixed->second,"count");if(!mixed_count||mixed_count->type!=types.store.number()||mixed_count->optional)fail("Mixed.count:number missing");
