...
enum class ExpressionKind { Unknown,Identifier,Literal,Parenthesized,Unary,Binary,Call,Property,ObjectLiteral,Assignment,Function };
...
class ExpressionModel {
 struct Builder {
  ...
  ExpressionId build(){if(sig.empty())return add(ExpressionKind::Unknown,0,0);if(tokens[sig[0]].text=="function")return function();int depth=0;for(std::size_t p=0;p<sig.size();++p){const auto&t=tokens[sig[p]].text;if(t=="("||t=="["||t=="{")++depth;else if(t==")"||t=="]"||t=="}"){if(depth)--depth;}else if(t=="=>"&&depth==0)return function(p);}auto root=assignment();if(pos!=sig.size())return add(ExpressionKind::Unknown,sig.front(),sig.back()+1,{}, {root});return root;}
 };
public:
