...
enum class ExpressionKind { Unknown,Identifier,Literal,Parenthesized,Unary,Binary,Call,New,Property,ObjectLiteral,ArrayLiteral,Assignment,Function };
...
class ExpressionModel {
 struct Builder {
  ...
  ExpressionId unary(){if(pos<sig.size()){const auto op=sig[pos];const auto&t=tokens[op].text;if(t=="new"){++pos;auto child=postfix();return add(ExpressionKind::New,op,model.nodes_[child].end_token,t,{child},op);}if(t=="+"||t=="-"||t=="~"||t=="!"||t=="typeof"){++pos;auto child=unary();return add(ExpressionKind::Unary,op,model.nodes_[child].end_token,t,{child},op);}}return postfix();}
