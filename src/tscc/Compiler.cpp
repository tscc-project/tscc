...
int compile_files(const std::vector<std::string>&roots,const CompilerOptions&o){
 ...
 const fs::path root=o.root_dir.empty()?common_root(roots):fs::path(o.root_dir);ProgramGraph graph;graph.build(roots,o.follow_imports,o.module_resolution,o.base_url,o.paths,o.root_dirs);d.append(graph.diagnostics());
