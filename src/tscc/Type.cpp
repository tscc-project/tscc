...
TypeModel build_type_model(const std::vector<Token>& tokens, const Program& program,
    ...
    for(const auto&syntax:program.root.children){
        ...
        auto header=name_token+1;if(header<open&&tokens[header].text=="<"){auto generic_close=matching(header,"<",">",open);std::size_t part=header+1;while(part<generic_close){auto comma=part;while(comma<generic_close&&tokens[comma].text!=",")++comma;auto parameter_name=next_token(part,comma);if(parameter_name<comma&&tokens[parameter_name].kind==TokenKind::Identifier){auto parameter=model.store.type_parameter(tokens[parameter_name].text);local_names[tokens[parameter_name].text]=parameter;info.type_parameters.push_back({tokens[parameter_name].text,parameter,model.store.unknown(),model.store.unknown()});}part=comma+1;}header=generic_close+1;}
        for(auto p=header;p<open;++p){if(tokens[p].text=="extends"){auto base=next_token(p+1,open);if(base<open)info.base_name=tokens[base].text;}if(tokens[p].text=="implements"){for(auto q=p+1;q<open;++q)if(tokens[q].kind==TokenKind::Identifier)info.implements_names.push_back(tokens[q].text);break;}}
