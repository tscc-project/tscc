...
TypeId annotation_type(const std::vector<Token>& tokens,
    ...
    if(first<declaration.type_end_token&&(tokens[first].text=="{"||tokens[first].text=="("))return parse_annotation(tokens,first,declaration.type_end_token,store,named);
...
const Type*TypeStore::callable(TypeId id)const{return kind(id)==TypeKind::Function&&id!=function()?&types_[id]:nullptr;}
