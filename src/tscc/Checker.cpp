...
private:
    ...
    TypeId type(ExpressionId id, TypeId contextual = 0) {
        ...
        if(node.kind==ExpressionKind::Property){
            ...
            const auto*property=types_.store.property(value,node.text);
        ...
        if(node.kind==ExpressionKind::Call){
            ...
            const auto&callee=expressions_.node(node.children[0]);TypeModel::FunctionSignature signature;
            if(callee.kind==ExpressionKind::Identifier){const auto symbol=binding_.symbol_for_reference(callee.operator_token);if(symbol>=types_.symbol_types.size())return types_.store.unknown();if(symbol<types_.function_signatures.size()&&types_.function_signatures[symbol].valid)signature=types_.function_signatures[symbol];else if(const auto*callable=types_.store.callable(types_.symbol_types[symbol])){signature.parameters=callable->parameters;signature.result=callable->result;signature.required_parameters=callable->required_parameters;signature.rest=callable->rest;signature.valid=true;}}
            else if(const auto*callable=types_.store.callable(type(node.children[0]))){signature.parameters=callable->parameters;signature.result=callable->result;signature.required_parameters=callable->required_parameters;signature.rest=callable->rest;signature.valid=true;}
            if(!signature.valid)return types_.store.unknown();
            std::vector<TypeId>pre_arguments;if(!signature.type_parameters.empty())for(std::size_t i=1;i<node.children.size();++i)pre_arguments.push_back(type(node.children[i]));
            if(!node.type_argument_ranges.empty()||!signature.type_parameters.empty()){
                ...
                if(chosen.size()>signature.type_parameters.size()&&error_.empty()){error_token_=callee.operator_token;error_="Expected "+std::to_string(signature.type_parameters.size())+" type arguments, but got "+std::to_string(chosen.size())+".";}
                ...
                std::unordered_map<TypeId,TypeId>map;for(std::size_t i=0;i<signature.type_parameters.size();++i){TypeId selected=i<chosen.size()?chosen[i]:types_.store.unknown();if(selected==types_.store.unknown()){auto found=candidates.find(signature.type_parameters[i].type);if(found!=candidates.end()){auto values=found->second;std::sort(values.begin(),values.end());values.erase(std::unique(values.begin(),values.end()),values.end());if(values.size()==1)selected=values.front();else if(values.size()>1&&error_.empty()){error_token_=callee.operator_token;error_="Conflicting inferences for type parameter '"+signature.type_parameters[i].name+"'.";}}}if(selected==types_.store.unknown())selected=signature.type_parameters[i].default_type;if(selected==types_.store.unknown()){if(error_.empty()){error_token_=callee.operator_token;error_="Could not infer type parameter '"+signature.type_parameters[i].name+"'.";}continue;}const auto constraint=signature.type_parameters[i].constraint;if(constraint!=types_.store.unknown()&&!types_.store.assignable(selected,types_.store.substitute(constraint,map))&&error_.empty()){error_token_=callee.operator_token;error_="Type argument '"+types_.store.name(selected)+"' does not satisfy constraint '"+types_.store.name(constraint)+"'.";}map[signature.type_parameters[i].type]=selected;}
                for(auto&parameter:signature.parameters)parameter=types_.store.substitute(parameter,map);
                signature.result=types_.store.substitute(signature.result,map);
            }
            std::vector<TypeId>arguments;for(std::size_t i=1;i<node.children.size();++i){const auto parameter=i-1<signature.parameters.size()?i-1:signature.parameters.empty()?0:signature.parameters.size()-1;const auto expected_argument=signature.parameters.empty()?types_.store.unknown():signature.parameters[parameter];auto actual=i-1<pre_arguments.size()?pre_arguments[i-1]:types_.store.unknown();if(actual==types_.store.unknown())actual=type(node.children[i],expected_argument);arguments.push_back(actual);}
            if((arguments.size()<signature.required_parameters||(!signature.rest&&arguments.size()>signature.parameters.size()))&&error_.empty()){error_token_=callee.operator_token;error_="Expected "+std::to_string(signature.parameters.size())+" arguments, but got "+std::to_string(arguments.size())+".";}
            else for(std::size_t i=0;i<arguments.size()&&!signature.parameters.empty();++i){const auto parameter=i<signature.parameters.size()?i:signature.parameters.size()-1,expected=signature.parameters[parameter],actual=arguments[i];if(expected!=types_.store.unknown()&&types_.store.kind(expected)!=TypeKind::TypeParameter&&actual!=types_.store.unknown()&&!types_.store.assignable(actual,expected)&&error_.empty()){error_token_=callee.operator_token;error_="Argument of type '"+types_.store.name(types_.store.widen(actual))+"' is not assignable to parameter of type '"+types_.store.name(expected)+"'.";}}
