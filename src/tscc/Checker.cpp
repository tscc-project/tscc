...
private:
    ...
    TypeId type(ExpressionId id, TypeId contextual = 0) {
        ...
        if(node.kind==ExpressionKind::Identifier){if(node.text=="undefined")return types_.store.undefined();if(node.text=="this"){for(const auto&info:types_.classes)if(node.operator_token>info.body_begin_token&&node.operator_token<info.body_end_token)return info.instance_type;return types_.store.unknown();}const auto symbol=binding_.symbol_for_reference(node.operator_token);auto contextual=contextual_facts_.find(symbol);if(contextual!=contextual_facts_.end())return contextual->second;if(facts_){auto fact=facts_->find(symbol);if(fact!=facts_->end())return fact->second;}return symbol<types_.symbol_types.size()?types_.symbol_types[symbol]:types_.store.unknown();}
        ...
        if(node.kind==ExpressionKind::Call){
            ...
            if(callee.kind==ExpressionKind::Identifier){const auto symbol=binding_.symbol_for_reference(callee.operator_token);if(symbol>=types_.symbol_types.size())return types_.store.unknown();if(symbol<types_.overload_sets.size()&&!types_.overload_sets[symbol].empty())signatures=types_.overload_sets[symbol];else if(const auto*callable=types_.store.callable(types_.symbol_types[symbol])){TypeModel::FunctionSignature signature;signature.parameters=callable->parameters;signature.result=callable->result;signature.required_parameters=callable->required_parameters;signature.rest=callable->rest;signature.valid=true;signatures.push_back(std::move(signature));}}
            else if(const auto*callable=types_.store.callable(type(node.children[0]))){TypeModel::FunctionSignature signature;signature.parameters=callable->parameters;signature.result=callable->result;signature.required_parameters=callable->required_parameters;signature.rest=callable->rest;signature.valid=true;signatures.push_back(std::move(signature));}
    ...
    TypeId type_new(ExpressionId id) {
        ...
        const auto&info=types_.classes[types_.symbol_classes[symbol]];auto signature=info.constructor_overloads.empty()?info.constructor:info.constructor_overloads.front();TypeId instance=info.instance_type;
        if(!node.type_argument_ranges.empty()&&!info.type_parameters.empty()){std::vector<TypeId>arguments;for(const auto&range:node.type_argument_ranges){auto at=range.first;arguments.push_back(parse_type_annotation(tokens_,at,range.second,types_.store,&types_.named_types));}bool valid=false;auto instantiated=types_.store.instantiate_generic(info.name,arguments,&valid);if(!valid){if(error_.empty()){error_token_=callee.operator_token;error_="Invalid type arguments for class '"+info.name+"'.";}return types_.store.unknown();}instance=instantiated;std::unordered_map<TypeId,TypeId>map;for(std::size_t i=0;i<arguments.size()&&i<info.type_parameters.size();++i)map[info.type_parameters[i].type]=arguments[i];for(auto&parameter:signature.parameters)parameter=types_.store.substitute(parameter,map);}
        const auto count=node.children.size()-1;if(count<signature.required_parameters||(!signature.rest&&count>signature.parameters.size())){if(error_.empty()){error_token_=callee.operator_token;error_="Expected "+std::to_string(signature.parameters.size())+" arguments, but got "+std::to_string(count)+".";}return instance;}for(std::size_t i=1;i<node.children.size()&&!signature.parameters.empty();++i){const auto parameter=i-1<signature.parameters.size()?i-1:signature.parameters.size()-1;const auto actual=type(node.children[i],signature.parameters[parameter]);if(actual!=types_.store.unknown()&&signature.parameters[parameter]!=types_.store.unknown()&&!types_.store.assignable(actual,signature.parameters[parameter])&&error_.empty()){error_token_=callee.operator_token;error_="Argument of type '"+types_.store.name(types_.store.widen(actual))+"' is not assignable to parameter of type '"+types_.store.name(signature.parameters[parameter])+"'.";}}return instance;
    }
