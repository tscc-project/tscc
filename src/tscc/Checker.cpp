    ...
    TypeId type(ExpressionId id, TypeId contextual = 0) {
        ...
        if(node.kind==ExpressionKind::Call){
            ...
            if(!node.type_argument_ranges.empty()||!signature.type_parameters.empty()){
                ...
                std::unordered_map<TypeId,TypeId>map;for(std::size_t i=0;i<signature.type_parameters.size();++i){auto selected=i<chosen.size()?chosen[i]:signature.type_parameters[i].default_type;if(selected==types_.store.unknown()){if(!node.type_argument_ranges.empty()&&error_.empty()){error_token_=callee.operator_token;error_="Generic type parameter '"+signature.type_parameters[i].name+"' requires an explicit type argument.";}continue;}const auto constraint=signature.type_parameters[i].constraint;if(constraint!=types_.store.unknown()&&!types_.store.assignable(selected,types_.store.substitute(constraint,map))&&error_.empty()){error_token_=callee.operator_token;error_="Type argument '"+types_.store.name(selected)+"' does not satisfy constraint '"+types_.store.name(constraint)+"'.";}map[signature.type_parameters[i].type]=selected;}
